...
class AbstractFilter(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.fft.irfft(torch.view_as_complex(projection.transpose(2,3)), n=projection.transpose(2,3).shape[3]).transpose(2,3)[:,:,:input_size,:]
    ...
    def _get_fourier_filter(self, size):
        ...
        fourier_filter = torch.view_as_real(torch.fft.fft(f))
...
class LearnableFilter(AbstractFilter):
    ...
    def forward(self, x):
        ...
        projection = torch.view_as_real(torch.fft.fft2(x.transpose(2,3), dim=0)).transpose(2,3) * fourier_filter
        return torch.fft.irfft(torch.view_as_complex(projection.transpose(2,3)), n = projection.transpose(2,3).shape[3]).transpose(2,3)[:,:,:input_size,:]